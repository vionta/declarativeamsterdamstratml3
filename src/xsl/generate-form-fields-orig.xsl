  ...
  <xsl:template match="/">
	...
	<plan-element>
	  <element-main-section>
	    <element-title>
	      StratML Plan <xf:input ref="Name" class="Source" placeholder="Name"  >
	      <!--	  <xf:label  >Plan Name</xf:label> -->
	    </xf:input>
	    <xf:input ref="instance('entity')/Code" class="Source"  >
...
<!--	<element-buttons>
		...
		<line>
		  ...
		  <xsl:apply-templates select="/*[1]/element" />
		</line>
	      </xf:repeat>
	    </element-content>
	  </element-main-section>
	...
	<element-main-section>
	  <element-title>
	    <xsl:if test="./element/name/text() = 'SequenceIndicator' " >
	      <xf:input ref="./SequenceIndicator" class="Source SequenceIndicator"  />
  ...
  <xsl:template  match="element[(name/text()) and not(type) and not(name/text() = 'SWOT') and not(name/text() = 'PESTLE') ]" mode="nonew" >
	...
	<element-main-section>
	  <element-title>
	    <xsl:if test="./element/name/text() = 'SequenceIndicator' " >
	      <xf:input ref="./Name" class="Source SequenceIndicator"  />
	      <xf:trigger ref=".[./SequenceIndicator]" >
		<xf:label>x</xf:label>
		<xf:action ev:event="DOMActivate">
		  <xf:delete ref="SequenceIndicator" />
